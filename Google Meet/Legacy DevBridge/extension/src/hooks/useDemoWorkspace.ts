...
export function useDemoWorkspace() {
  ...
  return { workspace, error, load, connect: () => perform(demoApi.connect), branch: (name: string) => perform(() => demoApi.branch(name)), commit: (message: string) => perform(() => demoApi.commit(message)), pullRequest: () => perform(demoApi.pullRequest) };
}
