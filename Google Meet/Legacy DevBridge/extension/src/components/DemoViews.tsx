...
function CodeAssistant() {
  ...
  const [conversation, setConversation] = useState<Array<{ role: "user" | "assistant"; text: string }>>([
    { role: "assistant", text: "I’m your project-aware Code Assistant. I can explain code, check security and company standards, and prepare fixes for your approval." },
  ]);
  const [result, setResult] = useState<AssistantResponse | null>(null);
  const send = async () => {
    const prompt = request.trim(); if (!prompt) return;
    setConversation((items) => [...items, { role: "user", text: prompt }]); setRequest("");
    const response = await demoApi.chat(prompt); setResult(response);
    setConversation((items) => [...items, { role: "assistant", text: response.message }]);
  };
  const decide = async (approved: boolean) => {
    if (!result?.proposal) return;
    const response = await demoApi.decide(result.proposal.proposal_id, approved); setResult(response);
    setConversation((items) => [...items, { role: "assistant", text: response.message }]);
  };
