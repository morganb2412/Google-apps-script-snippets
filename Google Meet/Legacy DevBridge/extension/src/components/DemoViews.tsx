...
import { agentApi, demoApi } from "../services/apiClient";
...
function Repository({ workspace, actions }: { workspace: DemoWorkspace; actions: Actions }) {
  ...
  const auditEvents = workspace.audit_events ?? [];
  return <><DemoLabel /><h2>Repository</h2><p className="repo-name">{workspace.repository ?? "No repository connected"}</p><p className="muted">Branch · {workspace.branch}</p>
    ...
    {workspace.latest_commit && <p className="success-line">Latest commit · {workspace.latest_commit}</p>}{workspace.pull_request_url && <p className="success-line"><a href={workspace.pull_request_url} target="_blank" rel="noreferrer">Demo pull request · #1 ready for review</a></p>}{auditEvents.length > 0 && <details><summary>Audit trail ({auditEvents.length})</summary>{auditEvents.map((event, index) => <p className="audit-event" key={`${event}-${index}`}>{event}</p>)}</details>}</>;
}
...
function CodeAssistant() {
  ...
  const send = async () => { const prompt = request.trim(); if (!prompt) return; setConversation((items) => [...items, { role: "user", text: prompt }]); setRequest(""); setBusy(true); setError(null); try { const response = await agentApi.chat(prompt); setResult(response); setConversation((items) => [...items, { role: "assistant", text: response.message }]); } catch (caught) { setError(caught instanceof Error ? caught.message : "The Code Assistant could not respond."); } finally { setBusy(false); } };
  ...
  return <><p className="live-label">MANAGED AI · VERTEX AI</p><h2>Code Assistant</h2><p className="muted">Conversational project help · security and standards aware · changes require approval</p><div className="conversation">{conversation.map((message, index) => <div key={`${message.role}-${index}`} className={`message message--${message.role}`}><strong>{message.role === "user" ? "You" : "DevBridge"}</strong><p>{message.text}</p></div>)}</div><textarea disabled={busy} aria-label="Message Code Assistant" value={request} onChange={(event) => setRequest(event.target.value)} placeholder="Ask about the project or request a fix…" /><button disabled={busy} className="primary primary--active" onClick={() => void send()}>{busy ? "Thinking…" : "Send"}</button>{error && <p className="action-error" role="alert">{error}</p>}{result?.provider && <p className="fine-print">Provider: {result.provider} · Model: {result.model}</p>}{result?.findings.length ? <div className="findings"><h3>Standards & security findings</h3>{result.findings.map((finding) => <p key={finding}>{finding}</p>)}</div> : null}{result?.proposal && <div className="proposal"><div className="proposal-head"><strong>Proposed change</strong><span>{result.proposal.risk_level} RISK</span></div><p>{result.proposal.file} · {result.proposal.operation}</p><p className="muted">{result.proposal.explanation}</p><pre className="diff">{result.proposal.diff}</pre><p className="fine-print">Original hash: {result.proposal.original_hash.slice(0, 16)}… · Status: {result.proposal.status}</p>{result.proposal.status === "PENDING_APPROVAL" && <div className="button-row"><button className="primary primary--active" onClick={() => void decide(true)}>Approve demo change</button><button className="secondary" onClick={() => void decide(false)}>Reject</button></div>}</div>}</>;
}
